...
@Name("Last Woken")
@Description("The world date when a villager last woke up.")
@Examples({
	"broadcast the last woke memory of last spawned villager",
	"set the last woke memory of last spawned villager to now",
	"add 5 seconds to the last woken memory of last spawned villager"
})
@Since("INSERT VERSION")
public class ExprMemoryLastWoken extends SimplePropertyExpression<LivingEntity, WorldDate> {

	private static final MemoryKey<Long> MEMORY_KEY = MemoryKey.LAST_WOKEN;

	static {
		Skript.registerExpression(ExprMemoryLastWoken.class, WorldDate.class, ExpressionType.PROPERTY,
			"[the] last woke[n] memory [of %livingentities%]",
			"[the] %livingentities%'[s] last woke[n] memory");
	}
