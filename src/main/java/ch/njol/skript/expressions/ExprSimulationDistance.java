...
@Description({
	...
	"Requires Paper server to change the simulation distance for both worlds and players.",
	"Due to unstable behavior on older versions, the changing of simulation distance for worlds is restricted to Paper 1.21+."
})
@Examples({
...
public class ExprSimulationDistance extends SimplePropertyExpression<Object, Integer> {
	...
	public void change(Event event, Object @Nullable [] delta, ChangeMode mode) {
		int provided = mode == ChangeMode.RESET ? Bukkit.getSimulationDistance() : delta == null ? 2 : (Integer) delta[0];
		Consumer<Player> playerConsumer;
		Consumer<World> worldConsumer;
		switch (mode) {
			case SET, DELETE, RESET -> {
				playerConsumer = player -> player.setSimulationDistance(Math2.fit(2, provided, 32));
				worldConsumer = world -> world.setSimulationDistance(Math2.fit(2, provided, 32));
			}
			case ADD -> {
				playerConsumer = player -> {
					int current = player.getSimulationDistance();
					player.setSimulationDistance(Math2.fit(2, current + provided, 32));
				};
				worldConsumer = world -> {
					int current = world.getSimulationDistance();
					world.setSimulationDistance(Math2.fit(2, current + provided, 32));
				};
			}
			case REMOVE -> {
				playerConsumer = player -> {
